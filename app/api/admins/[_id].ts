...
import { DBClient } from '@lib/server';
import { Admin } from '@models';
import { IAdmin } from '@types';
import { scrapeLinkWebsite } from '@lib/server';
  ...
  switch (method) {
    ...
    case 'PUT' /* Edit an admin by its ID */:
      try {
        ...
        const adminWithUpdatedLink = await Admin.findByIdAndUpdate(_id, link, {
          new: true,
        });
        if (!adminWithUpdatedLink) {
